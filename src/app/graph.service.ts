...
export class GraphService {
  ...
  Getchartinfo() {
    return this.http.get<any>("https://api.rootnet.in/covid19-in/stats/latest")
      .pipe(
        map((response: any) => {
          // Process the data to extract time series
          const regionalData = response.data.regional.map((item: any) => {
            return {
              loc: item.loc,
              totalConfirmed: item.totalConfirmed,
              totalRecovered: item.discharged, // Assuming discharged is totalRecovered
